...
template<typename T>
void sortArray(std::vector<T>& array, int (*vergleich)(T const*, T const*) = incComp<T>)
{
    qsort((void*)array.data(), array.size(), sizeof(T), (int (*)(void const*, void const*))vergleich);
}
