...
#define FUER_ALLE_KNOTEN(u, graph) for(size_t u = 0; u < (graph).anzKnoten(); ++u)
...
public:
    ...
    IndexPaar(size_t argKno = KEIN_INDEX, size_t argKan = KEIN_INDEX)
        : iKnoten(argKno)
        , iKante(argKan)
    { }
...
public:
    ...
    {
        if(v >= anzKnoten()) { throw "Graph::knoten(): Index zu gross!"; }
        return _knoten[v];
    }
    ...
    {
        if(e >= anzKanten()) { throw "Graph::kante(): Index zu gross!"; }
        return _kanten[e];
    }
    ...
    {
        if(v >= anzKnoten()) { throw "Graph::nachbarn(): Index zu gross!"; }
        return _nachbarn[v];
    }
    ...
    {
        if(v >= anzKnoten()) { throw "Graph::rnachbarn(): Index zu gross!"; }
    ...
    // wirf Exception, falls v ungültig
    size_t grad(size_t v) const { return nachbarn(v).size(); }


    // gib Rückwärts-Grad des Knotens v aus
    // wirf Exception, falls v ungültig
    size_t rgrad(size_t v) const { return rnachbarn(v).size(); }
    ...
    size_t setzeKante(std::string const& name, size_t u, size_t v);
